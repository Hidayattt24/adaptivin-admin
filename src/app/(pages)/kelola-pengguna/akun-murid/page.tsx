...
import BulkMoveModal from "@/components/user-management/BulkMoveModal";
...
export default function AkunMuridPage() {
  ...
  const handleResetPassword = async (user: ManagedUser) => {
    ...
    try {
      ...
      await Swal.fire({
        ...
        html: `Password baru untuk <b>${user.nama}</b>:<br/><br/><div style="background: #f3f4f6; padding: 12px; border-radius: 8px; font-size: 20px; font-weight: bold; color: #1f2937; letter-spacing: 2px;">${newPassword}</div><br/><small style="color: #6b7280;">Salin dan bagikan ke siswa yang bersangkutan</small>`,
        icon: "success",
  ...
  const handleDelete = async (user: ManagedUser) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "Terjadi kesalahan saat menghapus akun murid.";
      await Swal.fire({
        title: "Gagal",
        text: errorMessage,
        icon: "error",
  ...
  const handleBulkMove = () => {
    if (selectedUserIds.length === 0) {
      Swal.fire({
        ...
        text: "Centang checkbox siswa yang ingin dipindahkan terlebih dahulu",
        icon: "warning",
        confirmButtonColor: "#33A1E0",
        confirmButtonText: "OK",
        background: "#ffffff",
  ...
  const handleBulkMoveConfirm = async (targetKelasId: string) => {
    setIsBulkMoving(true);
    try {
      await bulkMoveStudents(selectedUserIds, targetKelasId);
      await loadUsers();
      setSelectedUserIds([]);
      ...
      await Swal.fire({
        title: "Berhasil!",
        text: `${selectedUserIds.length} siswa berhasil dipindahkan ke kelas baru.`,
        icon: "success",
        confirmButtonColor: "#33A1E0",
        confirmButtonText: "OK",
        background: "#ffffff",
      ...
      console.error("Failed to bulk move students:", error);

      let errorMessage = "Terjadi kesalahan saat memindahkan siswa.";
  ...
  const handleModalSubmit = async (submission: UserModalSubmission) => {
    try {
      ...
      if (error?.response?.data) {
        ...
        } else if (errorMessage.includes("NISN") && errorMessage.includes("already")) {
          errorMessage = "NISN sudah terdaftar. Gunakan NISN lain atau cek data yang sudah ada.";
        }
      } else if (error?.message) {
        errorMessage = error.message;
      }
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `export_siswa_${Date.now()}.xlsx`;
      ...
      await Swal.fire({
        ...
        text: "Data siswa berhasil di-export ke Excel",
        timer: 1500,
